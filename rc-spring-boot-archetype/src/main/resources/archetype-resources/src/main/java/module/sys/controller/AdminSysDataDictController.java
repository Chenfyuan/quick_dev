...
import ${package}.module.sys.service.impl.SysDataDictServiceImpl;
...
@RequestMapping(SysConstant.SYS_MODULE_CONTEXT_PATH + RcConstant.Version.HTTP_API_VERSION_V1 + "/sys/dataDicts")
@RestController
public class AdminSysDataDictController {
    ...
    @Resource
    private SysDataDictServiceImpl sysDataDictServiceImpl;
    ...
    @GetMapping
    public ApiResult<PageResult<SysDataDictVO>> list(PageParam pageParam, AdminListSysDataDictDTO dto) {
        return ApiResult.data(sysDataDictServiceImpl.adminList(pageParam, dto));
    }

    @SaCheckPermission(type = AdminStpUtil.TYPE, value = PERMISSION_PREFIX + RcConstant.Permission.RETRIEVE)
    @ApiOperation(value = "详情", produces = MediaType.APPLICATION_JSON_VALUE)
    @GetMapping(value = "/{id}")
    public ApiResult<SysDataDictVO> getById(@PathVariable Long id) {
        return ApiResult.data(sysDataDictServiceImpl.getOneById(id));
    }
    ...
    public ApiResult<?> insert(@RequestBody @Valid AdminInsertOrUpdateSysDataDictDTO dto) {
        sysDataDictServiceImpl.adminInsert(dto);
    ...
    public ApiResult<?> update(@PathVariable Long id, @RequestBody @Valid AdminInsertOrUpdateSysDataDictDTO dto) {
        dto.setId(id);
        sysDataDictServiceImpl.adminUpdate(dto);
    ...
    public ApiResult<?> delete(@RequestBody @Valid IdsDTO<Long> dto) {
        sysDataDictServiceImpl.adminDelete(dto.getIds());
